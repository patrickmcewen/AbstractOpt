"""Generate per-kernel evaluator scripts for OpenEvolve."""
...
def _get_stepdb_loader(stepdb_path: str):
    """Import StepDB's loader module."""
    stepdb = Path(stepdb_path).resolve()
    if str(stepdb) not in sys.path:
        sys.path.insert(0, str(stepdb))
    import loader as stepdb_loader
    return stepdb_loader
...
_EVALUATOR_TEMPLATE = '''\
"""Auto-generated evaluator script for {kernel_name} / {preset}."""
...
STEPDB_PATH = {stepdb_path_repr}
STEP_TL_SRC = {step_tl_src_repr}
STEP_TL_PROTO = {step_tl_proto_repr}
REFERENCE_PATH = {reference_path_repr}
DIMS = {dims_repr}
KERNEL_NAME = {kernel_name_repr}
PRESET = {preset_repr}

IMPORT_SCAFFOLD = {import_scaffold_repr}
...
def make_evaluator_script(
    ...
    """Generate a self-contained evaluator script for a kernel/preset pair.

    Args:
        kernel_name: kernel name
        preset: preset name
        dims: dimension dict (already resolved)
        reference_path: absolute path to reference.py
        stepdb_path: path to StepDB (for STeP imports resolution)
    """
    stepdb_resolved = Path(stepdb_path).resolve()
    step_tl_src = str(stepdb_resolved.parent / "step_tl" / "src")
    step_tl_proto = str(stepdb_resolved.parent / "step_tl" / "src" / "proto")

    return _EVALUATOR_TEMPLATE.format(
        kernel_name=kernel_name,
        preset=preset,
        stepdb_path_repr=repr(str(stepdb_resolved)),
        step_tl_src_repr=repr(step_tl_src),
        step_tl_proto_repr=repr(step_tl_proto),
        reference_path_repr=repr(str(Path(reference_path).resolve())),
        dims_repr=repr(dims),
        kernel_name_repr=repr(kernel_name),
        preset_repr=repr(preset),
        import_scaffold_repr=repr(IMPORT_SCAFFOLD),
    )
...
def write_evaluator(
    ...
    """Write a self-contained evaluator script to disk. Returns the output path."""
    script = make_evaluator_script(kernel_name, preset, dims, reference_path, stepdb_path)
