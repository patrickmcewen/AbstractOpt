...
class Index(Select):
    ...
    def size_in_bytes(self) -> sympy.Expr:
        ...
        return sympy.Integer(2 * self.total_n)
...
class Stream:
    ...
    def has_ragged_dim(self) -> bool:
        """Check if any dimension in shape contains a DynDim with is_ragged=True."""
        for dim in self.shape:
            if isinstance(dim, DynDim) and dim.is_ragged:
                return True
        return False
