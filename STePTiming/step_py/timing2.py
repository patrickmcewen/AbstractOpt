...
def _run_timing_pass(nodes, graph, hw_config, offchip_oti,
    ...
    for n in nodes:
        ...
        # Substitute DynDim expected values early so all rate computations
        # use concrete numbers (not deferred to final evaluation).
        if sym_subs:
            if hasattr(n_fire, 'free_symbols') and n_fire.free_symbols:
                n_fire = n_fire.subs(sym_subs)
            if hasattr(otpc, 'free_symbols') and otpc.free_symbols:
                otpc = otpc.subs(sym_subs)
            for k, v in nit_map.items():
                if hasattr(v, 'free_symbols') and v.free_symbols:
                    nit_map[k] = v.subs(sym_subs)

        from step_py.ops import Streamify as _Streamify
        if n.is_offchip_memory_op():
            t_fire = sympy.Integer(0)
        elif isinstance(n, _Streamify):
            # Streamify reads from PMU buffer, outputs OTPC subtiles to FIFO.
            # Sim does incr_cycles(1) per subtile, so T_fire = OTPC.
            t_fire = otpc
...
def _compute_contention_oti(offchip_nodes, offchip_requests, offchip_pardispatch,
                             hw_config, pass_info, has_symbolic, sym_subs=None):
...
def analyze_timing(graph: MultiDiGraph, hw_config: dict = None) -> dict:
    ...
    sym_subs = dict(sym_expected) if sym_expected else None
